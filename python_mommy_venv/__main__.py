...
def assert_venv(only_warn: bool = False):
    if not IS_VENV:
        mommy_logger.error("doesn't run in a virtual environment~")
...
def wrap_interpreter(path: Path, symlink_target: Path):
    ...
    mommy_logger.info("shows her girl where the interpreter is: %s -> %s", inner_symlink, symlink_target)
    serious_logger.info("creating symlink: %s -> %s", inner_symlink, symlink_target)
    inner_symlink.symlink_to(symlink_target)

    # remove original symlink
    mommy_logger.info("deletes the original interpreter~ %s", path)
    serious_logger.info("deleting original symlink %s", path)
    path.unlink()

    # creating the wrapper string
    mommy_logger.info("writes wrapper script as %s", path)
...
def install_pip_hook(path: Path):
    ...
    if "# GENERATED BY MOMMY" in text:
        mommy_logger.info("already watches %s", str(path))
        serious_logger.info("pip hook already installed at %s", str(path))
        return

    mommy_logger.info("needs to keep an eye on this little pip~ %s", str(path))
...
def mommify_venv(is_mommy: bool = True):
    ...
    mommy_logger.info("looks in %s to mess your system up~ <33", str(bin_path))
